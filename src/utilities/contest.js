...
export const studentPoints = (studentId, category, maxPoints) => {
  ...
  let points = 0;
  if (!category.data().runs) return 0;
  const releventRuns = category
    .data()
    .runs.filter((x) => x.student === studentId);
...
export const rank = (student, category) => {
  ...
  return {
    rank: studentIndex + 1,
    otherStudents: allPoints[studentIndex].students,
  };
};
...
export const studentCategory = (student, categories) => {
  const relevantCats = categories.filter((x) =>
    x
      .data()
      .students.map((x) => x.id)
      .includes(student.id)
